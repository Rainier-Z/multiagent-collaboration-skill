...
主讨论 Markdown 是讨论与正式决策内容权威；`.multiagent/audit/events.jsonl` 是追加式事实日志；state.json 是可恢复的当前快照；候选/正式 Word 是对应 Markdown 的只读快照。参与者不得写状态文件。所有状态变化必须先写产物/回执，再 append event，最后以事务方式更新快照；崩溃后无法恢复一致性时 fail-closed。
...
| security_mode | enum | `normal`（默认，仅路径/事件审计）或 `strict`（额外要求 Ed25519、平台证明与 stop proof） |
...
| event_stream | object | append-only JSONL 事件账本路径、最后事件/哈希与校验状态 |
| transaction_recovery | object | 最近事务日志路径、阶段与恢复/失配结果 |
...
当前协议阶段顺序为 initialized → independent_proposal → cross_response(round=N) → candidate_decision → human_review → finalizing → confirmed_decision → delivered；旧工作区仍兼容下方七阶段枚举。
...
候选 Word 打开、人工确认、正式结论发布、停止请求、正式 Word 打开和最终交付都须分别保留时间与证据；`human_review`、`finalizing`、`confirmed_decision` 是可恢复的显式阶段。
...
5. 监测停止与用户确认：候选打开后进入 human_review；用户确认后发布正式结论并进入 finalizing，再向全部参与者下发 stop。仅收齐唯一 completed 回执和有效证明后才设置 monitoring.enabled/status/stop_requested_at；用户确认另记原文和时间。
...
实现提供一个独立的 append-only JSONL 事件流，默认位于
`.multiagent/audit/events.jsonl`。每条记录包含单调递增的 `sequence`、唯一
`event_id`、`transaction_id`（事务事件）、`revision_before`、
`revision_after`、`previous_hash` 和 `event_hash`。读取或追加前会校验整条哈希链；
截断、重排、重复或哈希失配必须阻断，不得静默修复原始事件。

受控事务的可见提交顺序固定为：

1. 读取并验证 state revision、事件流和调用方前置条件；
2. 原子写入业务产物及 receipt，并在事务日志中记录原始哈希；
3. 追加一条绑定事务 ID 和前后 revision 的事件；
4. 原子更新 `.multiagent/state.json`，使 revision 恰好递增一次。

事务日志位于 `.multiagent/audit/transactions/<transaction_id>/journal.json`。
进程在事件追加前崩溃时，恢复入口回滚已写产物且不产生事件；事件已追加而 state
